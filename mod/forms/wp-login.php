<?php
	class WPLoginForm extends FormInterface
	{
		...
		{	
			...
			$this->routeData();
			$enabled_login_with_otp = websmslk_get_option( 'login_with_otp', 'Websms_general', 'on');
		...
		{
			//return (websmslk_get_option('buyer_login_otp', 'Websms_general')=="on") ? true : false; //commented on 01-07-2019
			
			return (websmslk_get_option('buyer_login_otp', 'Websms_general')=="on" || websmslk_get_option('login_with_otp', 'Websms_general')=="on") ? true : false;
		}
		...
		function byPassLogin($user_role)
		{
			...
			$excluded_roles 	= websmslk_get_option('admin_bypass_otp_login', 'Websms_general',array());
		...
		{
			return (websmslk_get_option('allow_multiple_user', 'Websms_general')=="on") ? true : false;
		}
		...
		function _handle_Websms_wp_login($user, $username, $password)
		{
			WebsmsUtility::checkSession();
			/*login with otp*/
			$login_with_otp_enabled = (websmslk_get_option('login_with_otp', 'Websms_general')=="on") ? true : false;
