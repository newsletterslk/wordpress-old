<?php
	class WooCommerceRegistrationForm extends FormInterface
	{
		...
		function handleForm()
		{
			$this->popupEnabled 	= (websmslk_get_option('register_otp_popup_enabled', 'Websms_general')=="on") ? TRUE : FALSE;
		...
		public function wc_user_created($user_id, $data)
		{
			...
			if(array_key_exists('billing_phone', $post_data))
			{
				...
				do_action('websmslk_after_update_new_user_phone',$user_id,$billing_phone);
			}
		}
		...
		function woocommerce_site_registration_errors($errors,$username,$email)
		{
			...
			if(websmslk_get_option('allow_multiple_user', 'Websms_general')!="on") {
