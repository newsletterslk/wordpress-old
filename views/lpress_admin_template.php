					 ...
					 foreach($lpress_statuses as $ks => $vs)
					 {
						...
						<div id="accordion_<?php echo $ks; ?>" class="cvt-accordion-body-content">
							<table class="form-table">
								...
								<textarea name="Websms_lpress_message[lpress_admin_sms_body_<?php echo $vs; ?>]" id="Websms_lpress_message[lpress_admin_sms_body_<?php echo $vs; ?>]" <?php echo((websmslk_get_option( 'lpress_admin_notification_'.$vs, 'Websms_lpress_general', 'on')=='on')?'' : "readonly='readonly'"); ?>><?php 
						  echo websmslk_get_option('lpress_admin_sms_body_'.$vs, 'Websms_lpress_message', defined('WebsmsMessages::DEFAULT_LPRESS_ADMIN_SMS_'.str_replace('-', '_', strtoupper($vs))) ? constant('WebsmsMessages::DEFAULT_LPRESS_ADMIN_SMS_'.str_replace('-', '_', strtoupper($vs))) : WebsmsMessages::DEFAULT_LPRESS_ADMIN_SMS_STATUS_CHANGED); 
