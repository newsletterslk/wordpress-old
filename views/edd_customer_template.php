...
<div class="cvt-accordion">
	...
	<?php 
	 foreach($edd_order_statuses as $ks => $vs)
	 {
		...
		<?php echo ((websmslk_get_option( 'edd_order_status_'.$vs, 'Websms_edd_general', 'on')=='on')?"checked='checked'":''); ?>/><label><?php _e( 'when Order is '.ucwords(str_replace('-', ' ', $vs )), WebsmsConstants::TEXT_DOMAIN ) ?></label>
		...
		<div id="accordion_cust_<?php echo $ks; ?>" class="cvt-accordion-body-content">
			<table class="form-table">
				...
				echo $vs; ?>]" <?php echo(($current_val==$vs)?'' : "readonly='readonly'"); ?>><?php 	
					...
					echo websmslk_get_option('edd_sms_body_'.$vs, 'Websms_edd_message', defined('WebsmsMessages::DEFAULT_EDD_BUYER_SMS_'.str_replace('-', '_', strtoupper($vs))) ? constant('WebsmsMessages::DEFAULT_EDD_BUYER_SMS_'.str_replace('-', '_', strtoupper($vs))) : WebsmsMessages::DEFAULT_EDD_BUYER_SMS_STATUS_CHANGED); ?></textarea>
				</td>
				</tr>
			</table>
		</div>
