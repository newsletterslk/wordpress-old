...
class WebsmsEdd
{
	   ...
	   public static function getEDDVariables()
	   {
		...
		public static function trigger_after_update_edd_status($payment_id)
		{
			...
			$admin_send = websmslk_get_option('edd_admin_notification_'.$status, 'Websms_edd_general');
			$cst_send = websmslk_get_option('edd_order_status_'.$status, 'Websms_edd_general');
			
			if($cst_send=='on')
			{
				$content = websmslk_get_option( 'edd_sms_body_'.$status, 'Websms_edd_message');
			...
			if($admin_send=='on')
			{
				...
				$content = websmslk_get_option( 'edd_admin_sms_body_'.$status, 'Websms_edd_message');
				$content = self::pharse_sms_body( $content, $payment_id);
