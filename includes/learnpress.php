...
class WebsmsLearnPress
{
		...
		public static  function Websms_lp_send_sms_user_enroll($course_id, $user_id, $user_course) {
			...
			$buyer_sms_notify = websmslk_get_option( 'course_enroll', 'Websms_lpress_general', 'on' );
			$admin_sms_notify = websmslk_get_option( 'admin_course_enroll', 'Websms_lpress_general', 'on' );
				
			if($buyer_sms_notify=='on')
			{
				
				$buyer_sms_content = websmslk_get_option( 'sms_body_course_enroll', 'Websms_lpress_message', '');
			...
			if($admin_sms_notify=='on')
			{
				...
				{
					$admin_sms_content = websmslk_get_option( 'sms_body_course_enroll_admin_msg', 'Websms_lpress_message', '');
		...
		public static function Websms_lp_user_course_finished($course_id, $user_id, $user_item_id) {
			...
			$buyer_sms_notify = websmslk_get_option( 'course_finished', 'Websms_lpress_general', 'on' );
			$admin_sms_notify = websmslk_get_option( 'admin_course_finished', 'Websms_lpress_general', 'on' );
				
			if($buyer_sms_notify=='on')
			{
				$buyer_sms_content = websmslk_get_option('sms_body_course_finished', 'Websms_lpress_message', '');
				WebsmscURLOTP::sendsms(array('number'=>$billing_phone,'sms_body'=>self::parse_sms_content(NULL,$buyer_sms_content,NULL,$user_id,$course_id)));
			}
			...
			if($admin_sms_notify=='on')
			{
				...
				if($admin_phone_number!='')
				{
					$admin_sms_content = websmslk_get_option( 'sms_body_course_finished_admin_msg', 'Websms_lpress_message', '');
					WebsmscURLOTP::sendsms(array('number'=>$admin_phone_number,'sms_body'=>self::parse_sms_content(NULL,$admin_sms_content,NULL,$user_id,$course_id)));
				}
			}
		}
		...
		public static  function Websms_lp_send_sms_on_changedStatus($order_id, $old_status, $new_status) {
			...
			{
				$buyer_sms_notify = websmslk_get_option( 'lpress_order_status_'.$new_status, 'Websms_lpress_general', 'on' );
				$admin_sms_notify = websmslk_get_option( 'lpress_admin_notification_'.$new_status, 'Websms_lpress_general', 'on' );
				$user_id  			= get_post_meta( $order_id, '_user_id', true );
				
				if($buyer_sms_notify=='on')
				{
					$buyer_sms_content = websmslk_get_option( 'lpress_sms_body_'.$new_status, 'Websms_lpress_message', WebsmsMessages::DEFAULT_LPRESS_BUYER_SMS_STATUS_CHANGED );
				...
				if($admin_sms_notify=='on')
				{
					...
					if($admin_phone_number!='')
					{
						$admin_sms_content = websmslk_get_option( 'lpress_admin_sms_body_'.$new_status, 'Websms_lpress_message', WebsmsMessages::DEFAULT_LPRESS_ADMIN_SMS_STATUS_CHANGED );
		...
		public static  function Websms_lp_after_become_teacher($user_id, $role, $old_roles) {
			...
			$buyer_sms_notify = websmslk_get_option( 'become_teacher', 'Websms_lpress_general', 'on' );
			if($buyer_sms_notify=='on')
			{
				$billing_phone = get_user_meta($user_id,'billing_phone',true);
				$buyer_sms_content= websmslk_get_option( 'sms_body_become_teacher_msg', 'Websms_lpress_message', '' );
				if($role=='lp_teacher')
				{
			...
			$admin_sms_notify = websmslk_get_option( 'admin_become_teacher', 'Websms_lpress_general', 'on' );
			if($admin_sms_notify=='on')
			{
				$admin_phone_number     = websmslk_get_option( 'sms_admin_phone', 'Websms_message', '' );
				$admin_sms_content= websmslk_get_option( 'sms_body_admin_become_teacher_msg', 'Websms_lpress_message', '' );
				if($role=='lp_teacher')
				{
